...
ALL_ENGINES = ("searxng", "duckduckgo", "startpage", "mojeek")
BASE_CONFIG = {"searxng_url": "https://endianness.de"}
FETCH_K = 20
VARIANTS = [
    ...
    {"name": "searxng_only", "engines": ["searxng"], "per_engine": 10, "rank": "full"},
]
...
def _fetch_engine(name, query):
    try:
        if name == "searxng":
            return engines.searxng(query, BASE_CONFIG["searxng_url"], FETCH_K)
        if name == "duckduckgo":
...
def _load_cache(here, query_id):
    with open(os.path.join(here, "cache", query_id + ".json"), encoding="utf-8") as handle:
        return json.load(handle)


def _pool_from_cache(cache, variant):
    allowed = variant["engines"] or ALL_ENGINES
...
def main():
    ...
    for variant in VARIANTS:
        ...
        with open(os.path.join(runs_dir, variant["name"] + ".json"), "w", encoding="utf-8") as handle:
