...
def strip_fenced(text):
    pattern = re.compile(r"(?ms)^" + re.escape(START) + r".*?^" + re.escape(END) + r"\n*")
...
def merge(config_path, snippet_path, repo):
    ...
    block = f"{snippet}\n" if START in snippet and END in snippet else f"{START}\n{snippet}\n{END}\n"
...
def main(argv):
    config_path = Path(argv[1]).expanduser() if len(argv) > 1 else Path.home() / ".codex" / "config.toml"
    snippet_path = Path(argv[2]).expanduser() if len(argv) > 2 else Path(__file__).with_name("codex-config.snippet.toml")
